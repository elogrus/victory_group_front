...
import { Users, Trash2, X, Plus, BrainCircuit, Loader2 } from "lucide-react";
...
export function AdminProjects({ projects, setProjects, users, roles }: any) {
    ...
    const handleOpenAnalytics = async (project: Project) => {
        ...
        try {
            ...
            setTimeout(() => {
                setAnalyticsData({ ...MOCK_ANALYTICS_DATA, projectId: project.id });
    ...
    return (
        <div className="space-y-6">
                ...
                <div key={proj.id} className="border rounded-xl bg-card shadow-sm overflow-hidden mb-6">
                    <div className="p-5 flex items-center justify-between border-b bg-muted/10">
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                <BrainCircuit className="w-4 h-4 mr-2"/> AI Аналитика
                            </Button>
                        ...
                        {(!proj.assignments || proj.assignments.length === 0) ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {proj.assignments.map(assign => {
                                    const u = users.find((x: User) => x.id === String(assign.userId));
                                    ...
                                    return (
                                        <div key={u.id} className="flex items-center justify-between p-3 border rounded-lg bg-background hover:border-blue-500/30 transition-all">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-100 text-[10px] uppercase font-bold">{r.name}</Badge>
            ...
            <Dialog open={analyticsModalOpen} onOpenChange={(open) => { if(!open) setAnalyticsModalOpen(false); }}>
                <DialogContent className="!max-w-[1200px] w-[90vw] h-[90vh] flex flex-col p-0 overflow-hidden [&>button]:hidden">
                    <div className="p-6 border-b flex justify-between items-center bg-background shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-foreground">Аналитический отчет AI</h2>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 bg-muted/5">
                        {isAnalyticsLoading ? (
                            <div className="h-full flex flex-col items-center justify-center gap-4">
                                ...
                                <p className="text-muted-foreground font-medium animate-pulse">Анализируем данные проекта и генерируем отчет...</p>
                            </div>
                        ) : (
                    ...
                    <div className="p-4 border-t bg-background flex justify-end shrink-0">
                        <Button onClick={() => setAnalyticsModalOpen(false)} className="bg-blue-600 text-white">Понятно</Button>
                    </div>
                </DialogContent>
            </Dialog>

            {/* Модалка назначения участника */}
            <Dialog open={assignModalOpen} onOpenChange={(open) => { if(!open) setAssignModalOpen(false); }}>
            ...
            <ConfirmModal 
                ...
                onConfirm={() => setProjects(projects.filter((p:any)=>p.id !== projToDelete))}
