...
export function Sidebar({ isOpen, setIsOpen, projects, activeProjId, onSelect, onNewProject }: any) {
    return (
        <aside className={`relative border-r bg-muted/10 transition-all duration-300 flex flex-col shrink-0 ${isOpen ? 'w-64' : 'w-16'}`}>
            <div className="p-4 flex items-center justify-between border-b h-14 shrink-0">
                {isOpen && <span className="font-semibold text-sm text-muted-foreground uppercase tracking-wider text-[10px]">Проекты</span>}
                {isOpen && <Button variant="ghost" size="icon" onClick={onNewProject} className="w-6 h-6 hover:bg-muted/50"><Plus className="w-4 h-4" /></Button>}
            </div>
            ...
            <nav className="p-2 flex-1 flex flex-col gap-1 overflow-y-auto overflow-x-hidden no-scrollbar">
                {projects.map((p: any) => {
                    ...
                    return (
                        ...
                        >
                            <FolderDot className={`w-5 h-5 shrink-0 ${isActive ? 'text-blue-600' : ''}`} />
                            {isOpen && <span className="truncate">{p.name}</span>}
                        </button>
                    );
                })}
            </nav>
