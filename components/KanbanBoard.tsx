...
import { SortableContext, useSortable, horizontalListSortingStrategy, arrayMove, verticalListSortingStrategy } from '@dnd-kit/sortable';
import { CSS } from '@dnd-kit/utilities';
import { Card, CardContent } from "@/components/ui/card";
import { Task, TaskStatus, COLUMNS as INITIAL_COLUMNS } from "../lib/data";
import { Check, User, GripHorizontal, Plus, CornerDownLeft } from "lucide-react";
      ...
      <Card className={`cursor-grab active:cursor-grabbing border bg-card mb-2 ${isOverlay ? 'shadow-xl rotate-2 scale-105 border-blue-500/50' : 'hover:border-blue-500/50 shadow-sm transition-colors'}`}>
        <CardContent className="p-3 flex flex-col gap-2">
          ...
          <div className="flex justify-between items-center mt-1">
            ...
            <div className="w-6 h-6 rounded-full bg-muted flex items-center justify-center border text-[10px] font-bold">DK</div>
          </div>
        </CardContent>
      </Card>
    </div>
...
// --- КОМПОНЕНТ КОЛОНКИ ---
function Column({ col, tasks, onTaskClick, updateColumnName, isOverlay, creatingInCol, setCreatingInCol, inlineTitle, setInlineTitle, handleInlineCreate }: any) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ 
    id: col.id, 
  ...
  if (isDragging && !isOverlay) {
    return (
      <div ref={setNodeRef} style={style} className="w-[300px] h-[500px] shrink-0 bg-blue-500/5 border-2 border-dashed border-blue-500/20 rounded-lg" />
    );
  }

  return (
    <div ref={setNodeRef} style={style} className={`flex flex-col w-[300px] shrink-0 bg-muted/30 rounded-lg p-2 h-max transition-shadow ${isOverlay ? 'shadow-2xl ring-1 ring-black/5 rotate-1' : ''}`}>
      {/* Шапка - ручка для перетаскивания */}
      <div {...attributes} {...listeners} className="p-2 flex items-center justify-between group cursor-grab active:cursor-grabbing">
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-tight">{col.title}</span>
        ...
        {creatingInCol === col.id ? (
          <div className="bg-card border-2 border-blue-500 rounded p-2 shadow-sm mt-1">
            <input 
              ...
              onKeyDown={e => { if (e.key === 'Enter') handleInlineCreate(col.id); if (e.key === 'Escape') setCreatingInCol(null); }}
              placeholder="Что нужно сделать?" className="w-full text-sm bg-transparent outline-none text-foreground mb-2" 
            />
...
export function KanbanBoard({ tasks, setTasks, columns, setColumns, onTaskClick }: any) {
  ...
  const handleInlineCreate = (colId: string) => {
    ...
    const newTask: Task = {
      id: `KAN-${Date.now().toString().slice(-4)}`, title: inlineTitle, description: '', status: colId as TaskStatus,
      priority: 'Medium', author: 'Diniar Karimov', assignee: null, createdAt: new Date().toLocaleDateString(), updatedAt: 'Только что'
    };
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={onDragStart} onDragEnd={onDragEnd}>
      <div className="flex gap-4 h-full overflow-x-auto pb-4 items-start scrollbar-hide">
        ...
        <button 
          onClick={() => setColumns([...columns, { id: `col-${Date.now()}`, title: 'Новая колонка' }])} 
