...
import { CreateProjectModal } from "@/features/Dashboard/ui/CreateProjectModal";
import { CreateBoardModal } from "@/features/Dashboard/ui/CreateBoardModal";
...
export default function Dashboard() {
    ...
    const [activeBoardId, setActiveBoardId] = useState(
        INITIAL_PROJECTS[0].boards[0].id,
    );
    ...
    const handleDeleteBoard = (boardId: string) => {
        ...
        if (
            confirm(
                "Вы уверены? Все задачи на этой доске будут безвозвратно удалены.",
            )
        ) {
            setProjects((prev) =>
                prev.map((p) =>
            ...
            if (activeBoardId === boardId) {
                const remainingBoards = activeProject.boards.filter(
                    (b) => b.id !== boardId,
                );
    ...
    const handleCreateTask = (data: { title: string; tags: string[] }) => {
        const newTask: Task = {
            ...
            title: data.title,
            description: "",
            status: activeBoard.columns[0]?.id || "todo",
            priority: "Medium",
